...
function ProductImageGallery({ productId, onImageChange }) {
  ...
  const fetchImages = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setImages(data);
        // ✅ اگر تصاویر وجود دارد، اولین تصویر را به عنوان انتخاب‌شده تنظیم کن
        if (data.length > 0) {
  ...
  const moveImage = async (index, direction) => {
    ...
    // ❌ جلوگیری از جابه‌جایی تصویر اصلی (id=0)
                  ...
                  {!isLegacy && (
                    <>
                      ...
                      <button
                        ...
                        disabled={index === 0 || (images[index - 1] && images[index - 1].id === 0)}
                        className={`p-1 rounded text-white text-xs transition ${
                          index === 0 || (images[index - 1] && images[index - 1].id === 0)
