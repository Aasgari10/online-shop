...
// ============================================================
// ✅ تابع pickBestVariation — نسخه اصلاح‌شده
// اولویت اول: variations با موجودی مثبت
// اولویت دوم: اگه همه ناموجود → ارزان‌ترین
// ============================================================
const pickBestVariation = (variationsWithPrice) => {
  if (!variationsWithPrice || variationsWithPrice.length === 0) return null;

  // ✅ مرحله ۱: variations با قیمت معتبر و موجودی مثبت
  const inStockValidVariations = variationsWithPrice.filter(v => 
    v.final_price > 0 && v.original_price > 0 && (v.stock || 0) > 0
  );

  // ✅ اگه variation با موجودی مثبت هست، از بین اونا انتخاب کن
  if (inStockValidVariations.length > 0) {
    const discounted = inStockValidVariations.filter(v => v.discount_applied);
    if (discounted.length > 0) {
      discounted.sort((a, b) => a.final_price - b.final_price);
      return discounted[0];
    }
    const sorted = [...inStockValidVariations].sort((a, b) => a.final_price - b.final_price);
    return sorted[0];
  }

  // ✅ مرحله ۲: اگه هیچ variation با موجودی نبود، fallback
  const validVariations = variationsWithPrice.filter(v => 
    v.final_price > 0 && v.original_price > 0
  );

  let pool = validVariations;
  if (pool.length === 0) {
    pool = variationsWithPrice.filter(v => (v.stock || 0) > 0);
  }
  if (pool.length === 0) {
    pool = variationsWithPrice;
  }

  const discounted = pool.filter(v => v.discount_applied);
  if (discounted.length > 0) {
    discounted.sort((a, b) => a.final_price - b.final_price);
    return discounted[0];
  }

  const sorted = [...pool].sort((a, b) => a.final_price - b.final_price);
  return sorted[0];
};

// ============================================================
// ✅ getProducts (نسخه نهایی)
// ============================================================
const getProducts = async (filters) => {
  ...
  // ✅ FIX 2: ترتیب صعودی وقتی فیلتر ستاره فعاله
  let orderBy;
  if (ratingFilter !== null) {
    orderBy = 'average_rating ASC, id ASC';
  } else {
    orderBy = sortMap[sort] || 'created_at DESC, id DESC';
  }
